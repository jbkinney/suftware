...
def derivative_matrix_1d(G, grid_spacing):
    ...
    return np.asmatrix(right_partial)
...
def laplacian_1d(G, alpha, grid_spacing, periodic, sparse=True, report_kernel=True):
    ...
    if periodic:
        tmp_mat = 2*np.diag(np.ones(G),0) - np.diag(np.ones(G-1),-1) - np.diag(np.ones(G-1),+1)
        tmp_mat[G-1,0] = -1.0
        tmp_mat[0,G-1] = -1.0
        Delta = (np.asmatrix(tmp_mat)/(grid_spacing**2))**alpha
...
def laplacian_2d( num_gridpoints, alpha, grid_spacing=[1.0,1.0], periodic=False, sparse=False, report_kernel=False):
    ...
    I_x = np.asmatrix(np.identity(Gx))
    I_y = np.asmatrix(np.identity(Gy))
    ...
    if periodic:
        ...
        Delta_1 = np.asmatrix(np.kron(Delta_x,I_y) + np.kron(I_x,Delta_y))
    ...
    # If bilateral, construct alpha-order bilateral laplacian algorithmically
        ...
        for a in range(alpha+1):
            ...
            coeff = comb(alpha,a)
            if a == 0:
                Delta = coeff*np.asmatrix(np.kron(Dx,Dy))
            else:
                Delta += coeff*np.asmatrix(np.kron(Dx,Dy))
