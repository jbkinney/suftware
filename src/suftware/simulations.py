...
def simulate_density(name):
    ...
    check(name in density_dict.keys(),
          ...
          "\n\t%s" % (name, ', '.join(density_names)))

    return density_dict[name]

@handle_errors
def simulate_dataset(name='DoubleGaussian', num_data_points=100, seed=None):
    """
    ...
    distribution: (str)
        The distribution from which to draw data. Run sw.list_simulated_densities()
    ...
    """
    check(name is not None, f"name must be a string; is {name}")
    check(isinstance(name, str), f"name must be a string; is {name}")
    check(isinstance(num_data_points, int), f"num_data_points must be an integer; is {num_data_points}")
    check(0 < num_data_points <= 1E6, f"num_data_points must be > 0 and <= 1E6; is {num_data_points}")
    if seed is not None:
        check(isinstance(seed, int), f"seed must be an integer; is {seed}")
        check(0 <= seed <= 2**32-1, f"seed must be an integer between 0 and 2**32-1; is {seed}")
