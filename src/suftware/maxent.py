...
def coeffs_to_field(coeffs, kernel):
    ...
    # Convert to matrices
    kernel_mat = np.asmatrix(kernel) # G x kernel_dim matrix
    coeffs_col = np.asmatrix(coeffs).T # kernel_dim x 1 matrix
...
def gradient_per_datum_from_coeffs(coeffs, R, kernel, phi0=False, 
    ...
    R_row = np.asmatrix(R) # 1 x G
    quasiQ_row = np.asmatrix(quasiQ) # 1 x G
    kernel_mat = np.asmatrix(kernel) # G x kernel_dim
    ...
    if regularized:
        reg_row = (1./G)*np.asmatrix(phi)/(PHI_STD_REG**2) # 1 x G
...
def hessian_per_datum_from_coeffs(coeffs, R, kernel, phi0=False, 
    ...
    kernel_mat = np.asmatrix(kernel) # G x kernel_dim 
    H = np.asmatrix(np.diag(quasiQ)) # G x G
